(function() {
    ...
    if (!modal || !messageEl || !btnCancel || !btnYes) return;
    ...
    window.showCustomConfirm = function(message, callback) {
        ...
        if (isDestructive) {
            if (iconDiv) {
                ...
                iconDiv.style.boxShadow = '0 8px 20px rgba(239, 68, 68, 0.3)';
            }
            ...
            btnYes.style.boxShadow = '0 4px 12px rgba(239, 68, 68, 0.2)';
            btnYes.textContent = 'Yes, Proceed';
        } else if (/log out|logout|Ø®Ø±ÙˆØ¬/i.test(message.toLowerCase())) {
            if (iconDiv) {
                iconDiv.style.background = 'linear-gradient(135deg, #f59e0b, #d97706)';
                iconDiv.style.boxShadow = '0 8px 20px rgba(245, 158, 11, 0.3)';
            }
            if (iconElement) iconElement.className = 'fas fa-sign-out-alt';
            btnYes.style.background = 'linear-gradient(135deg, #f59e0b, #d97706)';
            btnYes.style.boxShadow = '0 4px 12px rgba(245, 158, 11, 0.2)';
            btnYes.textContent = 'Yes, Logout';
        } else {
            if (iconDiv) {
                iconDiv.style.background = 'linear-gradient(135deg, #5b21b6, #7c3aed)';
                iconDiv.style.boxShadow = '0 8px 20px rgba(124, 58, 237, 0.3)';
            }
            if (iconElement) iconElement.className = 'fas fa-magic';
            btnYes.style.background = 'linear-gradient(135deg, #5b21b6, #7c3aed)';
            btnYes.style.boxShadow = '0 4px 12px rgba(124, 58, 237, 0.2)';
            btnYes.textContent = 'Yes, Proceed';
        }
